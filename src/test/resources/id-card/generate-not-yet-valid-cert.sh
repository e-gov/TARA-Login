...
apt-get update && apt-get install -y libfaketime openssl
...
LD_PRELOAD=/usr/lib/$(uname -m)-linux-gnu/faketime/libfaketime.so.1 FAKETIME="+50y" openssl x509 -req -days 1826 -in csr.pem -signkey privatekey.pem -out public.crt -extfile extensions.conf
