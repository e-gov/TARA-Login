...
public class SmartIdNotificationBasedController {
    ...
    public String authSidInit(@Validated @ModelAttribute(value = "credential") SidCredential sidCredential, Model model,
                              TaraSession taraSession) {
        log.info("Initiating Smart-ID authentication session");
        validateSession(taraSession);
        String verificationCode = authSidNotificationBasedService.startSidAuthSession(taraSession, sidCredential.getIdCode());
