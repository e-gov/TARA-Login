...
public class TaraErrorController implements ErrorController {
    ...
    public String handleHtmlError(Model model, WebRequest webRequest, @SessionAttribute(value = TARA_SESSION, required = false) TaraSession taraSession) {
        ...
        if (webRequest.getParameter("error_hint") != null)
            model.addAttribute("message", webRequest.getParameter("error_hint"));
        return "error";
    }

    @RequestMapping("/error")
    public ResponseEntity<Map<String, Object>> handleError(WebRequest webRequest, @SessionAttribute(value = TARA_SESSION, required = false) TaraSession taraSession) {
