jQuery(function ($) {
    ...
    try {
        ...
        if (eidasOnlyMode) {
            activateTab($('.c-tab-login__nav-link[data-tab="eu-citizen"]'), $('.c-tab-login__content[data-tab="eu-citizen"]'), $('.c-tab-login__warning[data-tab="eu-citizen"]'));
        } else {
    ...
    $(document).on('click', '.c-tab-login__nav-link', function(event){
      ...
      handleNavLinkClick($(this).data('tab'));
    });
    ...
    // Back button link
    $(document).on('click', 'form.c-form button.back', function (event) {
      event.preventDefault();

      handleNavLinkClick("eu-citizen");
    });
    ...
    function handleNavLinkClick(active) {
      ...
      activateTab($('.c-tab-login__nav-link[data-tab="' + active + '"]'), content, $('.c-tab-login__warning[data-tab="' + active + '"]'));
      try {
          // In some cases exception could be thrown while accessing localStorage, see https://github.com/Modernizr/Modernizr/blob/v3.11.6/feature-detects/storage/localstorage.js
      ...
      if (docwidth <= 800 ) {
        $('body').addClass('is-mobile-subview');
        $('.c-tab-login__nav-link[data-tab="' + active + '"]').parent().addClass('is-active');
      }
    ...
    // ID-card form submit
    $('#idCardForm button.submit').on('click', function(event){
      event.preventDefault();
    ...
    $('#mobileIdForm input.form-control').on('keypress', function(event){
      if (event.keyCode === 13) { // Enter key
        $('#mobileIdForm button.submit').trigger('click');
        event.preventDefault();
      }
    });
    ...
    $('#smartIdForm input.form-control').on('keypress', function(event){
      if (event.keyCode === 13) { // Enter key
        $('#smartIdForm button.submit').trigger('click');
        event.preventDefault();
      }
    });
    ...
    $('#eidasForm').on('click', 'a.method-btn', function(event){
      ...
      if (validateSelectizeValue($('#eidasForm select'), function(value){ return value; })) {
        ...
        if (["id-card", "smart-id", "mobile-id"].indexOf(value) > -1) {
    ...
    function activateMethodContent(country, method){
      ...
      methodContent.find("button.back").removeClass("hidden");
      ...
      switch(link.attr("data-tab")) {
        ...
        case "eu-citizen":
          if (countrySelect !== undefined) {
            ...
            try {
              // In some cases exception could be thrown while accessing localStorage, see https://github.com/Modernizr/Modernizr/blob/v3.11.6/feature-detects/storage/localstorage.js
              selected = localStorage.getItem('selected-country', "");
            } catch (e) {
            }
            countrySelect.setValue(selected);
            if (selected === "") {
