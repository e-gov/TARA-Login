jQuery(function ($) {
    ...
    if (supportsSmartIdApp()) {
        // Mobile device (capable of running Smart-ID app)
        $('[data-innerhtml-sid-device]').each(function () {
            const innerHtml = $(this).data('innerhtmlSidDevice');
            $(this).html(innerHtml);
        });
    } else {
        // Desktop device (Smart-ID app not available)
    ...
    try {
        ...
        activateTab(
            $('.c-tab-login__nav-link').first(),
            $('.c-tab-login__content').first(),
            $('.c-tab-login__warning').first()
        );
    }
    ...
    function handleIdCardBackendError(responseJson) {
        ...
        if (responseJson.reportable === true) {
            ...
            const errorReportNotificationMessage = $('#error-report-notification')
                .text()
    ...
    $('#eidasForm button.c-btn--primary').on('click', function (event) {
        ...
        if (
            validateSelectizeValue($('#eidasForm select'), function (value) {
                return value;
            })
        ) {
            $('#eidasForm').submit();
    ...
    $('#eidasForm select').on('change', function () {
        validateSelectizeValue($(this), function () {
            return true;
        });
    });
    ...
    function activateIdCardView(viewName) {
        ...
        const visibleElementsInViews = {
            ...
            reportableError: [
                ...
                errorReportUrlSelector,
                errorIncidentNumberWrapperSelector,
            ].join(','),
