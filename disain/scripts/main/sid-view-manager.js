jQuery(function ($) {
    ...
    (function () {
        const mainEl = $('[data-sid-login-form]');
        if (mainEl.length === 0) {
            return;
        }
        showEl(mainEl);
    ...
    function getViewElsByName(mainEl) {
        const els = mainEl.find('[data-sid-login-form-view]');
        const result = {};
        els.each(function () {
            result[$(this).attr('data-sid-login-form-view')] = $(this);
        });
        return result;
    }
    ...
    function setClickHandler(mainEl, action, handler) {
        mainEl.find(`[data-sid-login-form-action="${action}"]`).on('click', handler);
    }
